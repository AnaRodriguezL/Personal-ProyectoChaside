    ...
    validarSesion.validarSesionEstudiante(req.session, function (estado) {
        if (estado == false) {
            ...
            var paramsRender = {
                ...
                jsFile: '<script src="assets/js/estudiante/resultados.js" type="text/javascript"></script>',
                logoEmpresa: req.session.logoEmpresa,
                fotoUsuario: req.session.fotoUsuario,
            };
            res.render("estudiante/resultados", paramsRender);
        }
    });
});
...
router.post("/verResultado", function (req, res) {
    validarSesion.validarSesionEstudiante(req.session, function (estado) {
        if (estado == false) {
            req.session.destroy();
            res.send({ STATE: "FALSE" });
            ...
            var idPrueba = req.body.idPrueba;
            req.session.idPreubaResultado=idPrueba
